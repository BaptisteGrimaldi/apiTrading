...
export function moyenneMedianeResultDmi(resultSuccess, resultFail) {
    const moyenneVariationSuccess1 = [];
    const moyenneVariationSuccessResult = [];
    const moyenneGapHautSuccess = [];
    const moyenneGapBasSuccess = [];
    try {
    ...
    console.log('chance de réussite :' +
        ((moyenneVariationSuccess1.length /
            (moyenneVariationSuccess1.length + moyenneVariationFail1.length)) *
            100 +
            '%'));
    const moyenneVariationSuccessResult1 = moyenneVariationSuccess1.reduce((a, b) => a + b, 0) /
        moyenneVariationSuccess1.length;
    const moyenneVariationSuccessResultReal = moyenneVariationSuccessResult.reduce((a, b) => a + b, 0) /
        moyenneVariationSuccessResult.length;
    const moyenneGapHautSuccessResult = moyenneGapHautSuccess.reduce((a, b) => a + b, 0) /
        moyenneGapHautSuccess.length;
    const moyenneGapBasSuccessResult = moyenneGapBasSuccess.reduce((a, b) => a + b, 0) /
        moyenneGapBasSuccess.length;
    const moyenneVariationFailResult1 = moyenneVariationFail1.reduce((a, b) => a + b, 0) /
        moyenneVariationFail1.length;
    const moyenneVariationFailResult2 = moyenneVariationFail2.reduce((a, b) => a + b, 0) /
        moyenneVariationFail2.length;
    const moyenneGapHautFailResult = moyenneGapHautFail.reduce((a, b) => a + b, 0) /
        moyenneGapHautFail.length;
    const moyenneGapBasFailResult = moyenneGapBasFail.reduce((a, b) => a + b, 0) /
