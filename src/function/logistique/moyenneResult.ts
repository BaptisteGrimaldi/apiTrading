...
export function moyenneResult(resultSucess: dataResultBackTesting[], resultFail: dataResultBackTesting[]) {
  const moyenneVariationSucess1: number[] = [];

  const moyenneVariationSucess2: number[] = [];
  ...
  console.log(
    'chance de réussite :' + (moyenneVariationSucess1.length / (moyenneVariationSucess1.length + moyenneVariationFail1.length)) * 100 + '%'
  );
